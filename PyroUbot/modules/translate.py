import os
from gc import get_objects
import gtts
from gpytranslate import Translator
from pykeyboard import InlineKeyboard
from PyroUbot import *

__MODULE__ = "ᴛʀᴀɴꜱʟᴀᴛᴇ"
__HELP__ = """
<blockquote><b>⦪ ʙᴀɴᴛᴜᴀɴ ᴜɴᴛᴜᴋ ᴛʀᴀɴꜱʟᴀᴛᴇ ⦫</b>

<b>⎆ ᴘᴇʀɪɴᴛᴀʜ :</b>
ᚗ <code>{0}ᴛʀ</code> → ᴍᴇɴᴇʀᴊᴇᴍᴀʜᴋᴀɴ ᴘᴇsᴀɴ/ᴛᴇᴋs.
ᚗ <code>{0}ᴛᴛs</code> → ᴍᴇʀᴜʙᴀʜ ᴛᴇᴋs ᴍᴇɴᴊᴀᴅɪ ᴘᴇsᴀɴ sᴜᴀʀᴀ.
ᚗ <code>{0}sᴇᴛʟᴀɴɢ</code> → ᴍᴇʀᴜʙᴀʜ ʙᴀʜᴀsᴀ ᴛʀᴀɴsʟᴀᴛᴇ.

<b>⌭ ᴘᴇɴᴊᴇʟᴀsᴀɴ:</b>
ᚗ ᴀᴛᴜʀ ʙᴀʜᴀsᴀ ᴛᴜᴊᴜᴀɴ ᴛᴇʀʟᴇʙɪʜ ᴅᴀʜᴜʟᴜ ᴍᴇʟᴀʟᴜɪ sᴇᴛʟᴀɴɢ.</blockquote>
"""

@PY.UBOT("tts")
@PY.TOP_CMD
async def _(client, message):
    TM = await message.reply("<blockquote><b>🔍 sᴇᴅᴀɴɢ ᴍᴇᴍᴘʀᴏsᴇs, ᴍᴏʜᴏɴ ᴛᴜɴɢɢᴜ...</b></blockquote>")
    if message.reply_to_message:
        language = client._translate[client.me.id]
        words_to_say = message.reply_to_message.text or message.reply_to_message.caption
    else:
        if len(message.command) < 2:
            return await TM.edit(f"<blockquote><b>❌ ꜰᴏʀᴍᴀᴛ: <code>{message.text}</code> [ʀᴇᴘʟʏ/ᴛᴇᴋs]</b></blockquote>")
        else:
            language = client._translate[client.me.id]
            words_to_say = message.text.split(None, 1)[1]
            
    try:
        speech = gtts.gTTS(words_to_say, lang=language)
        speech.save("text_to_speech.oog")
        rep = message.reply_to_message or message
        await client.send_voice(
            chat_id=message.chat.id,
            voice="text_to_speech.oog",
            reply_to_message_id=rep.id,
        )
        await TM.delete()
    except Exception as error:
        await TM.edit(f"<blockquote><b>⚠️ ᴇʀʀᴏʀ:</b> <code>{error}</code></blockquote>")
    finally:
        if os.path.exists("text_to_speech.oog"):
            os.remove("text_to_speech.oog")


@PY.UBOT("tr")
@PY.TOP_CMD
async def _(client, message):
    trans = Translator()
    TM = await message.reply("<blockquote><b>🔍 sᴇᴅᴀɴɢ ᴍᴇɴᴇʀᴊᴇᴍᴀʜᴋᴀɴ...</b></blockquote>")
    if message.reply_to_message:
        dest = client._translate[client.me.id]
        to_translate = message.reply_to_message.text or message.reply_to_message.caption
        source = await trans.detect(to_translate)
    else:
        if len(message.command) < 2:
            return await TM.edit(f"<blockquote><b>❌ ꜰᴏʀᴍᴀᴛ: <code>{message.text}</code> [ʀᴇᴘʟʏ/ᴛᴇᴋs]</b></blockquote>")
        else:
            dest = client._translate[client.me.id]
            to_translate = message.text.split(None, 1)[1]
            source = await trans.detect(to_translate)
            
    try:
        translation = await trans(to_translate, sourcelang=source, targetlang=dest)
        reply = f"<blockquote>{translation.text}</blockquote>"
        rep = message.reply_to_message or message
        await TM.delete()
        await client.send_message(message.chat.id, reply, reply_to_message_id=rep.id)
    except Exception as error:
        await TM.edit(f"<blockquote><b>⚠️ ᴇʀʀᴏʀ:</b> <code>{error}</code></blockquote>")


@PY.UBOT("setlang")
@PY.TOP_CMD
async def _(client, message):
    query = id(message)
    try:
        x = await client.get_inline_bot_results(bot.me.username, f"ubah_bahasa {query}")
        return await message.reply_inline_bot_result(x.query_id, x.results[0].id)
    except Exception as error:
        return await message.reply(f"<blockquote><b>⚠️ ᴇʀʀᴏʀ:</b> <code>{error}</code></blockquote>")


@PY.INLINE("^ubah_bahasa")
async def _(client, inline_query):
    buttons = InlineKeyboard(row_width=3)
    keyboard = []
    for X in lang_code_translate:
        keyboard.append(
            InlineKeyboardButton(
                Fonts.smallcap(X.lower()),
                callback_data=f"set_bahasa {int(inline_query.query.split()[1])} {X}",
            )
        )
    buttons.add(*keyboard)
    await client.answer_inline_query(
        inline_query.id,
        cache_time=0,
        results=[
            (
                InlineQueryResultArticle(
                    title="ᴘɪʟɪʜ ʙᴀʜᴀsᴀ!",
                    reply_markup=buttons,
                    input_message_content=InputTextMessageContent(
                        "<blockquote><b>sɪʟᴀᴋᴀɴ ᴘɪʟɪʜ ʙᴀʜᴀsᴀ ᴛʀᴀɴsʟᴀᴛᴇ:</b></blockquote>"
                    ),
                )
            )
        ],
    )


@PY.CALLBACK("^set_bahasa")
async def _(client, callback_query):
    data = callback_query.data.split()
    try:
        m = [obj for obj in get_objects() if id(obj) == int(data[1])][0]
        m._client._translate[m._client.me.id] = lang_code_translate[data[2]]
        return await callback_query.edit_message_text(
            f"<blockquote><b>✅ ʙᴇʀʜᴀsɪʟ ᴅɪᴜʙᴀʜ ᴋᴇ: {Fonts.smallcap(data[2].lower())}</b></blockquote>"
        )
    except Exception as error:
        return await callback_query.edit_message_text(f"<blockquote><b>⚠️ ᴇʀʀᴏʀ:</b> <code>{error}</code></blockquote>")
        